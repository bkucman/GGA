public static Point[] ReadFile()
        ...
        public static Point[] FindMaxMin(Point[] arrayOfPoints)
        {
            ...
            // Jeśli nie ma jakiegoś extremum to szukam najwyższego lub najniższego pkt
            if (foundExtremes[0] == null)
            {
        ...
        public static bool IsKernelExist(Point[] extremes)
        {
           ...
           Console.WriteLine("Jądro istnieje ");

            return true;
        }

        static public Point[] Przecinanie1(Point[] extremes, Point[] points)
        {
            ...
            for (int j = 0; j < 2; j++) // dla min i max
            {
                ...
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    if ((points[i].x != extremes[j].x || points[i].y != extremes[j].y) && (points[(i + 1 + points.Length) % points.Length].x != extremes[j].x || points[(i + 1 + points.Length) % points.Length].y != extremes[j].y)) // pomijam odcinki zawierające max lub min zależnie od tego, które sprawdzam
                    {
                        if (j == 0 &&  !((points[i].y == extremes[j].y && points[(i + 1 + points.Length) % points.Length].y > extremes[j].y))) // sprawdzenie czy nie bierze odcinka który zaczyna się na prostej ale drugi koniec ma ponad minimum ,ten mnie nie interesuje
                        {
                            ...
                            if (signPQR != signPQS && signRSP != signRSQ)
                                //if( minPrzeciecie[0] == null)
                                {
                                    ...
                                    kmin += 2;
                                Console.WriteLine("Przecięcie odcinka " + points[i] + " " + points[(i + 1 + points.Length) % points.Length]);
                            } //else if (points[i].x - minPrzeciecie[0].x && points[(i + 1 + points.Length) % points.Length] <= minPrzeciecie[1].x)
                                

                        }else if (j == 1 && !( (points[i].y == extremes[j].y && points[(i + 1 + points.Length) % points.Length].y < extremes[j].y))) // sprawdzenie czy nie bierze odcinka który zaczyna się na prostej ale drugi koniec ma pod maximum ,ten mnie nie interesuje
                        {
                            var signPQR = Orientation(MakeMatrix(leftBorder, rightBorder, points[i]));
                            var signPQS = Orientation(MakeMatrix(leftBorder, rightBorder, points[(i + 1 + points.Length) % points.Length]));
                            var signRSP = Orientation(MakeMatrix(points[i], points[(i + 1 + points.Length) % points.Length], leftBorder));
            ...
            //Teraz muszę sprawdzić które brać pod uwagę xD jak mam więcej nie dwa odcinki przecięcia na stronę 

            return minPrzeciecie;
        }
        ...
        static public double[] prz(Point a, Point b, Point c, Point d) // ,pże działa
        {
        ...
        // zanjduje najbardziej wysunięty x na lewo i prawo, sposób na około do sprawdzania przecinania się prostej z odcinkiem :(
        ...
        public static void abc(Point[] points, Point[] extremas)
        {
            ...
            for (int i = 0; i < j + 2; i++)
            {
                if (founded[i].x == extremas[0].x && founded[i].y == extremas[0].y)
                {
                    obwod = 
                }
            }
